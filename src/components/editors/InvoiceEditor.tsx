...
export default function InvoiceEditor() {
  ...
  const updateFrom = (field: string, value: string) =>
    setData(d => ({ ...d, from: { ...d.from, [field]: value } }))

  const updateTo = (field: string, value: string) =>
    setData(d => ({ ...d, to: { ...d.to, [field]: value } }))
  ...
  const addLineItem = () =>
    setData(d => ({ ...d, line_items: [...d.line_items, { description: '', qty: 1, rate: 0 }] }))

  const removeLineItem = (i: number) =>
    setData(d => ({ ...d, line_items: d.line_items.filter((_, idx) => idx !== i) }))
      ...
      <h2 className="font-semibold text-gray-800">Invoice Details</h2>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <Input value={data.invoice_number} onChange={e => setData(d => ({ ...d, invoice_number: e.target.value }))} />
          </div>
          <div>
            <Label className="text-xs text-gray-500">Issue Date</Label>
            <Input type="date" value={data.issue_date} onChange={e => setData(d => ({ ...d, issue_date: e.target.value }))} />
          </div>
          <div className="col-span-2">
            <Label className="text-xs text-gray-500">Due Date</Label>
            <Input type="date" value={data.due_date} onChange={e => setData(d => ({ ...d, due_date: e.target.value }))} />
          </div>
        </div>

        <div className="space-y-2">
          <p className="text-xs font-semibold text-gray-400 uppercase">From</p>
          <Input placeholder="Your name" value={data.from.name} onChange={e => updateFrom('name', e.target.value)} />
          <Input placeholder="Your email" value={data.from.email} onChange={e => updateFrom('email', e.target.value)} />
          <Input placeholder="Your address" value={data.from.address} onChange={e => updateFrom('address', e.target.value)} />
        </div>

        <div className="space-y-2">
          <p className="text-xs font-semibold text-gray-400 uppercase">Bill To</p>
          <Input placeholder="Client name" value={data.to.name} onChange={e => updateTo('name', e.target.value)} />
          <Input placeholder="Client email" value={data.to.email} onChange={e => updateTo('email', e.target.value)} />
          <Input placeholder="Client address" value={data.to.address} onChange={e => updateTo('address', e.target.value)} />
        </div>

        <div className="space-y-2">
          <p className="text-xs font-semibold text-gray-400 uppercase">Line Items</p>
        ...
        <div className="flex gap-2 pt-2">
          ...
          <Button onClick={handleSaveAndShare} className="flex-1" disabled={saving}>
            {saving ? <Loader2 className="h-4 w-4 mr-1 animate-spin" /> : <Share2 className="h-4 w-4 mr-1" />}
            Share
          </Button>
        </div>
    ...
    <div className="bg-white rounded-xl border p-6 md:p-10 max-w-2xl mx-auto shadow-sm print:shadow-none">
          <div className="flex justify-between items-start mb-10">
            <div>
              ...
              <p className="text-gray-400 mt-1">#{data.invoice_number}</p>
            </div>
          ...
          <div className="grid grid-cols-2 gap-8 mb-10">
            ...
            <div>
              ...
              <p className="text-gray-500 text-sm">{data.to.address}</p>
            </div>
          </div>
            ...
            <tbody>
              {data.line_items.map((item, i) => (
                <tr key={i} className="border-b">
                  <td className="py-3 text-gray-700">{item.description || <span className="text-gray-300 italic">Item description</span>}</td>
                  ...
                  <td className="py-3 text-right text-gray-800 font-medium">${(item.qty * item.rate).toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </table>
